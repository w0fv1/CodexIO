...
describe('channel output delivery', () => {
  it('acknowledges a message only after the selected output completes', async () => {
    ...
    let settled = false
    const sending = manager.send(message()).then((result) => {
      settled = true
      return result
    })
  ...
  it('returns output failures to the message producer', async () => {
    ...
    await expect(manager.send(message())).resolves.toMatchObject({
      isFailed: true,
      message: 'web: web unavailable'
    })
  })

  it('delivers agent messages to every enabled output', async () => {
    const sent: string[] = []
  ...
  it('delivers one immutable message only once and retries failures', async () => {
    let attempts = 0
    ...
    expect((await manager.send(message())).isFailed).toBe(true)
    expect((await manager.send(message())).isFailed).toBe(false)
    expect((await manager.send(message())).isFailed).toBe(false)

    expect(web.send).toHaveBeenCalledTimes(2)
  })

  it('does not replace an existing identity with different content', async () => {
    const web = output('web')
    const manager = managerWith(web)
    const first = message()
    const second = {
    ...
    expect((await manager.send(first)).isFailed).toBe(false)
    expect((await manager.send(second)).isFailed).toBe(false)
    expect((await manager.send(first)).isFailed).toBe(false)

    expect(web.send).toHaveBeenCalledTimes(1)
    expect(vi.mocked(web.send).mock.calls[0][0].text).toBe('hello')
  })
